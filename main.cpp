...
void command(){
    ...
    struct slre rNegative, rPositive, setVel;
    slre_compile(&rNegative, "R-(\d*)"); 
    slre_compile(&rPositive, "R(\d*)"); 
    if(slre_match(&rNegative, receivedChars, 3, captures)){
        spin = true; 
        lead = -2;
    }
    else if(slre_match(&rPositive, receivedChars, 3, captures)){
        spin = true; 
        lead = 2; 
    }        
}
...
//uncomment the following part, comment out the previous code in the main,
//and set the set_velocity global variable to set the velocity
//because we haven't implemented the V commands
/*
    timer.start();
    runMotorThread.start(runMotor);
    interruptI1.rise(&incrementRev_findVel);
